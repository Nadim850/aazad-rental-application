...
export default function NotificationBell() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (!token) return;
      const res = await apiFetch(`${API_URL}/api/accounts/notifications/`, {
  ...
  const markAsRead = async (id, actionUrl) => {
    try {
      const token = localStorage.getItem("access");
      await apiFetch(`${API_URL}/api/accounts/notifications/${id}/read/`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const markAllAsRead = async () => {
    try {
      const token = localStorage.getItem("access");
      await apiFetch(`${API_URL}/api/accounts/notifications/read-all/`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const deleteNotification = async (e, id) => {
    ...
    try {
      const token = localStorage.getItem("access");
      await apiFetch(`${API_URL}/api/accounts/notifications/${id}/delete/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1">
                        <h4
                          className={`text-sm ${!notification.is_read ? "font-semibold" : "font-medium text-text-main/80"}`}
                        >
                          {notification.title}
                        </h4>
