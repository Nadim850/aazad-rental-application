            ...
            <div className="border border-border-main relative flex-1 overflow-hidden shadow-sm flex flex-col justify-end p-8 text-white">
              ...
              {facilityPlans.length > 1 && (
                <div className="absolute top-4 right-4 flex gap-2 z-10">
                  <button onClick={handlePrevPlan} className="bg-black/40 backdrop-blur-md p-2 border border-white/20 hover:bg-white/20 text-white transition-colors">
                    <ArrowLeft className="w-4 h-4" />
                  </button>
                  <button onClick={handleNextPlan} className="bg-black/40 backdrop-blur-md p-2 border border-white/20 hover:bg-white/20 text-white transition-colors">
                          ...
                          <div className="flex items-end justify-between mb-6">
                            <div className="flex flex-col">
                              <div className="flex items-baseline tracking-tight">
                                <span className="text-4xl font-bold">₹{totalPrice.toLocaleString('en-IN')}</span>
                                <span className="text-sm text-white/80 ml-1">/{duration}{duration === 1 ? 'mo' : 'mo'}</span>
                              </div>
                              {savingsPct > 0 && (
                                <div className="text-sm text-white/60 line-through mt-0.5 flex items-center gap-3">
                                  ...
                                  <span className="text-success font-bold text-xs no-underline bg-success/20 px-2 py-0.5 rounded-sm border border-success/30">Save {savingsPct}%</span>
                                </div>
                              )}
                            </div>
                          </div>
