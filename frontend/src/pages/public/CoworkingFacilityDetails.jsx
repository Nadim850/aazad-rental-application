...
export default function CoworkingFacilityDetails() {
  ...
  const facilityPlans = plans.filter(p => p.workspace_type === (targetFacilityId === 'dedicated' ? 'coworking' : targetFacilityId === 'cabin' ? 'coworking' : facility.id));
  const activePlan = facilityPlans.find(p => p.name.toLowerCase().includes(targetFacilityId === 'dedicated' ? 'desk' : 'cabin')) || facilityPlans[0];
      ...
      <div className="pt-24 pb-6 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto w-full">
        ...
        {isBookingMode && (
          <div className="mb-4">
            ...
            <p className="text-text-main/70 mt-2 text-sm">Review details and confirm your reservation below.</p>
          </div>
        )}
      </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-center space-y-10 lg:pl-8">
            ...
            <div className="border-t border-dashed border-border-main pt-8 grid grid-cols-2 gap-4">
               ...
               <div className="p-4 bg-surface rounded-xl border border-border-main">
                 ...
                 <p className="text-xs text-text-main/70">{facility.cancelPolicy}</p>
               </div>
            </div>
          </div>
