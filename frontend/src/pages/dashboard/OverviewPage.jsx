...
export default function OverviewPage() {
  ...
  const getWorkspaceTitle = (type) => {
    if (type === 'library') return 'Silent Reader Plan';
    if (type === 'dedicated') return 'Dedicated Desk';
    if (type === 'cabin') return 'Private Cabin';
    if (type === 'startup' || type === 'conference') return 'Conference Room';
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Current allocated seating and subscription info
              </CardDescription>
            </div>
            <Button
              variant="outline"
                ...
                if (active_subscription) {
                  ...
                  try {
                    ...
                    const availableSeats = data.filter(
                      (ws) => ws.workspace_type === wsType && ws.is_available,
                    );

                    if (availableSeats.length === 0) {
                      toast.error(
                        `Sorry, no seats are currently available for ${wsType}.`,
                      );
                      return;
                    }
                  } catch (err) {
                  ...
                  navigate(
                    `/pricing?seat=${active_subscription.workspace.name}&plan=${wsType}`,
                  );
                } else {
                  navigate("/pricing");
                }
            ...
            {active_subscription ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-xl">
                      {getWorkspaceTitle(active_subscription.workspace.workspace_type)}
                    </h3>
                  ...
                  <div className="grid grid-cols-2 gap-y-3 mb-4 text-sm text-text-main/70 mt-4">
                    <div className="flex items-center">
                      <MapPin className="w-4 h-4 mr-2" /> Floor 1, Main Wing
                    </div>
                    <div className="flex items-center">
                      <Clock className="w-4 h-4 mr-2" />{" "}
                      {active_subscription.access_hours || "9 AM - 9 PM"} Access
                    </div>
                    <div className="flex items-center">
                      <Calendar className="w-4 h-4 mr-2" /> Purchased:{" "}
                      {new Date(
                        active_subscription.start_time,
                      ).toLocaleDateString()}
                    </div>
                    <div
                      className={cn(
                        "flex items-center",
                        isNearExpire ? "text-error font-medium" : "",
                      )}
                    >
                      <Calendar
                        className={cn(
              ...
              <div className="text-center py-12 px-4 rounded-2xl bg-surface border border-border-main border-dashed">
                ...
                <p className="text-text-main/70 mb-6">
                  You don't have an active workspace booking yet.
                </p>
            ...
            {upcoming_subscriptions && upcoming_subscriptions.length > 0 && (
              <div className="mt-8 mb-6">
                <h3 className="text-lg font-semibold mb-4">
                  Upcoming Subscription(s)
                </h3>
                <div className="space-y-4">
                  {upcoming_subscriptions.map((upcoming) => (
                    <div
                      key={upcoming.id}
                      className="flex flex-col sm:flex-row gap-4 p-5 rounded-xl border border-primary/20 bg-primary/5"
                    >
                      <div className="w-16 h-16 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 border border-primary/20">
                        <span className="text-xl font-bold text-primary">
                          {upcoming.workspace.name}
                        </span>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-medium text-lg">
                            {getWorkspaceTitle(upcoming.workspace.workspace_type)}
                          </h4>
                        ...
                        <div className="text-sm text-text-main/70">
                          ...
                          <p>
                            Expires on:{" "}
                            {new Date(upcoming.end_time).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="mt-8">
                ...
                <div className="space-y-4">
                  {subscription_history.map((history) => (
                    <div
                      key={history.id}
                      className="flex items-center justify-between p-4 rounded-xl border border-border-main/50 bg-background/50 opacity-80 hover:opacity-100 transition-opacity"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-medium text-sm">
                            {getWorkspaceTitle(history.workspace.workspace_type)}
                          </p>
                          ...
                          <p className="text-xs text-text-main/60">
                            Valid:{" "}
                            {new Date(history.start_time).toLocaleDateString()}{" "}
                      ...
                      <Badge
                        variant={
                          history.status === "UPGRADED" ? "primary" : "outline"
                        }
                        className="text-[10px]"
                      >
                        {history.status}
          ...
          <CardContent>
            <div className="space-y-4">
              {payment_history && payment_history.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="font-medium text-sm">
                        ...
                        {parseFloat(
                          invoice.amount_paid ||
                            invoice.workspace.price_per_hour * 8 * 30,
                        ).toFixed(0)}
                      </p>
                      <Badge
                        variant="success"
                ...
                <p className="text-sm text-text-main/50 text-center py-4">
