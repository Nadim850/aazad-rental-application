...
export default function AdminUsersPage({ category = "library" }) {
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem("access");
      const res = await apiFetch(
        `${API_URL}/api/bookings/admin/users/${userId}/`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const activeSeat = (
      u.active_subscription?.workspace?.name || ""
    ).toLowerCase();
    const upcomingSeats = (u.upcoming_subscriptions || [])
      .map((sub) => (sub.workspace?.name || "").toLowerCase())
      .join(" ");
              ...
              <div className="flex justify-between items-start mb-2">
                <div>
                  <p className="font-semibold">
                    {user.active_subscription.workspace.name}
                  </p>
                  <p className="text-xs text-text-main/60 capitalize">
      ...
      <div>
        ...
        <p className="text-text-main/60">
          ...
          {category === "library"
            ...
            : "with dedicated, startup, or cabin subscriptions."}
        </p>
      </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl flex items-center gap-2 text-primary">
                    <Crown size={24} className="text-primary" /> Active
                    Subscribers
                  </CardTitle>
                  <p className="text-sm text-text-main/60 mt-1">
                    Users currently on an active plan.
                  </p>
                </div>
                <Badge
                  variant="primary"
                ...
                >
                  ...
                  {activeSubscribers.length === 1 ? "User" : "Users"}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {activeSubscribers.length === 0 ? (
                <div className="py-12 text-center text-text-main/50">
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 flex-1">
                            <div>
                              <p className="text-[10px] uppercase font-bold text-text-main/40 mb-1">
                                Plan
                              </p>
                              <p className="text-sm font-semibold">
                                {planName}
                              </p>
                              <p className="text-xs text-text-main/60 capitalize flex items-center gap-1 mt-0.5">
                                <Hash size={10} />
                                {workspaceType} ({sub.workspace.name})
                              </p>
                            </div>
                            <div>
                              <p className="text-[10px] uppercase font-bold text-text-main/40 mb-1">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl flex items-center gap-2">
                    <User size={24} className="text-text-main/70" /> Users
                    Without a Subscription
                  </CardTitle>
                  <p className="text-sm text-text-main/60 mt-1">
                    Registered users with no currently active plans.
                  </p>
                </div>
                <Badge
                  variant="outline"
                  className="text-base px-3 py-1 shadow-sm bg-background"
                >
                  {inactiveUsers.length}{" "}
                  {inactiveUsers.length === 1 ? "User" : "Users"}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="p-0">
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 md:flex-[0.7]">
                          ...
                          <div className="flex flex-col items-start md:items-end justify-center">
                            ...
                            <Badge
                              variant="outline"
                              className="px-2 py-0.5 text-text-main/60 border-text-main/20"
                            >
                              No Subscription
                            </Badge>
                          </div>
